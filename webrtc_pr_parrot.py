...
class WebRTCSyncPusher(FrameProcessor):
    def __init__(self, transport, model_pipeline, **kwargs):
        ...
        # Mic input queue and event loop reference
    ...
    async def _text_input_loop(self):
        ...
        # Set event loop for the mic thread and start Moonshine
        self.loop = asyncio.get_event_loop()
        if self.moonshine_thread is None:
            self.moonshine_thread = threading.Thread(
                target=self._moonshine_thread_func,
                daemon=True
            )
            self.moonshine_thread.start()

        logger.info("=" * 60)
        logger.info("Room connected. Type OR speak.")
        logger.info("=" * 60)
        ...
        while True:
            try:
                ...
                while True:
                    # Mic input - only if fresh (within 3 seconds)
                    while not self.mic_queue.empty():
                        item = self.mic_queue.get_nowait()
                        if isinstance(item, tuple):
                            text, ts = item
                            if time.time() - ts > 3.0:
                                continue  # stale phantom, drop it
                            user_text = text
                        else:
                            user_text = item
    ...
    def _moonshine_thread_func(self):
        """Background thread that runs Moonshine STT with debounce."""
        try:
            logger.info("Moonshine STT starting...")

            # Debounce state
            self._pending_text = [None]  # list so it's mutable in closure
            self._pending_timer = [None]

            def _fire_final():
                text = self._pending_text[0]
                self._pending_text[0] = None
                if text:
                    logger.info(f"VOICE-FINAL: {text}")
                    try:
                        if self.loop is not None:
                            asyncio.run_coroutine_threadsafe(
                                self.mic_queue.put((text, time.time())),
                                self.loop
                            )
                    except Exception as e:
                        logger.error(f"Failed to queue voice text: {e}")

            def on_text(text):
                if text and text.strip():
                    self._pending_text[0] = text.strip()
                    # Cancel previous timer if any
                    if self._pending_timer[0] is not None:
                        self._pending_timer[0].cancel()
                    # Start a new 1.5s timer
                    self._pending_timer[0] = threading.Timer(1.5, _fire_final)
                    self._pending_timer[0].daemon = True
                    self._pending_timer[0].start()

            def on_error(err):
                logger.error(f"Moonshine error: {err}")
            ...
            transcriber.load()
            transcriber.start()
            logger.info("Moonshine STT listening (1.5s debounce)...")
        except Exception as e:
            logger.error(f"Moonshine thread error: {e}")
