...
class WebRTCSyncPusher(FrameProcessor):
    ...
    async def _text_input_loop(self):
        ...
        if not self.chat_handler_registered:
            ...
            asyncio.create_task(self._chat_processor_loop())
    ...
    def register_chat_handler(self):
        ...
        try:
            ...
            def on_chat_message(reader, participant_identity):
                # LiveKit calls this synchronously - schedule async work
    ...
    async def _handle_chat_message(self, reader, participant_identity):
        try:
            ...
            logger.info(f"CHAT RECEIVED from {participant_identity}: {text}")
            await self.chat_queue.put((participant_identity, text))
        except Exception as e:
            logger.error(f"Chat handler error: {e}")

    async def _chat_processor_loop(self):
        logger.info("Chat processor loop started.")
        while True:
            try:
                identity, text = await self.chat_queue.get()
                logger.info(f"Processing chat from {identity}")
                await self._process_user_text(text, f"chat:{identity}")
            except Exception as e:
    ...
    async def _process_user_text(self, user_text, source):
        try:
            ...
            logger.info("Speech queued (from chat).")
        except Exception as e:
