...
class WebRTCSyncPusher(FrameProcessor):
    def __init__(self, transport, model_pipeline, **kwargs):
        ...
        # Chat queue for LiveKit text messages
        self.chat_queue = asyncio.Queue()
        self.chat_handler_registered = False
    ...
    async def _process_user_text(self, user_text, source):
        """Process a user message: AI reply -> send to chat -> TTS -> SoulX"""
        logger.info(f"[{source}] {user_text}")
        ...
        # Send reply back to LiveKit chat so guests see it
        try:
            if hasattr(self.transport, "send_message"):
                await self.transport.send_message(reply)
                logger.info("Sent reply to LiveKit chat")
        except Exception as e:
            logger.warning(f"Could not send chat reply: {e}")
    ...
    async def _text_input_loop(self):
        # Waits for room, then races terminal input vs LiveKit chat
        ...
        if not self.chat_handler_registered:
            self.register_chat_handler()

        logger.info("=" * 60)
        logger.info("Room connected. Type here OR type in Meet chat.")
        logger.info("=" * 60)
        ...
        while True:
            try:
                ...
                chat_task = asyncio.create_task(self.chat_queue.get())

                done, pending = await asyncio.wait(
                    [terminal_task, chat_task],
                ...
                if terminal_task in done:
                    user_text = terminal_task.result()
                    source = "terminal"
                elif chat_task in done:
                    identity, chat_text = chat_task.result()
                    user_text = chat_text
                    source = f"chat:{identity}"

                if not user_text or not user_text.strip():
                    continue

                try:
                    await self._process_user_text(user_text, source)
                except Exception as e:
    ...
    def register_chat_handler(self):
        """Register handler for LiveKit text stream (chat) messages."""
        try:
            room = self.transport._client._room
            if room is None:
                logger.warning("Room not available, chat handler not registered")
                return
            def on_chat_message(reader, participant_identity):
                # LiveKit calls this synchronously - schedule the async work
                asyncio.create_task(self._handle_chat_message(reader, participant_identity))
            room.register_text_stream_handler("lk.chat", on_chat_message)
            self.chat_handler_registered = True
            logger.info("Registered chat handler for topic 'lk.chat'")
        except Exception as e:
            logger.error(f"Failed to register chat handler: {e}")

    async def _handle_chat_message(self, reader, participant_identity):
        try:
            text = await reader.read_all()
            logger.info(f"Chat from {participant_identity}: {text}")
            await self.chat_queue.put((participant_identity, text))
        except Exception as e:
