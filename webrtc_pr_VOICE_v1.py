...
class WebRTCSyncPusher(FrameProcessor):
    ...
    async def _text_input_loop(self):
        ...
        while True:
            try:
                # Mic input takes priority if available
    ...
    def _moonshine_thread_func(self):
        """Background thread that runs Moonshine STT and pushes text to mic_queue."""
        try:
            logger.info("Moonshine STT starting...")
            transcriber = MicTranscriber(language="en")
            ...
            transcriber.start()
            logger.info("Moonshine STT listening...")
        except Exception as e:
